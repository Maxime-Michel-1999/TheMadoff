...
import MadoffNFT from '../contracts/ethereum-contracts/MadoffNFT.json'
...
export default function Home() {
  ...
  async function loadNFTs() {
    ...
    const nfts = await Promise.all(listings.map(async (i) => {
      try {
        const madoffNFTContract = new web3.eth.Contract(MadoffNFT.abi, MadoffNFT.networks[networkId].address)
        const tokenURI = await madoffNFTContract.methods.tokenURI(i.tokenId).call()
  ...
  async function buyNft(nft) {
    ...
    await marketPlaceContract.methods.buyNft(MadoffNFT.networks[networkId].address, nft.tokenId).send({ from: accounts[0], value: nft.price });
    loadNFTs()
  }
